...
public class CompaniesDBDAO implements CompaniesDAO {
    ...
    public boolean isIDExists(int id) {
        ...
        companyFields.put(1, id);
        try {
            resultset = (ResultSet) DataBaseUtils.runQueryForResult(GET_ONE_COMPANY, companyFields);
            if (resultset.next()) {
                return true;
            }
        } catch (SQLException error) {
    ...
    public void deleteCompany(int companyId) throws CouponSystemException {
        try {
            ...
            DataBaseUtils.runQuery(DELETE_COMPANY, companyFields);
        } catch (SQLException error) {
            System.out.println(error.getMessage());
            throw new CouponSystemException(CompanyErrMsg.NOT_EXISTS);
        }
    ...
    public Company getOneCompany(int companyID) throws CouponSystemException {
        try {
            Map<Integer, Object> companyFields = new HashMap<>();
            companyFields.put(1, companyID);
            ResultSet resultSet = (ResultSet) DataBaseUtils.runQueryForResult(GET_ONE_COMPANY, companyFields);
            resultSet.next();
            Company company = Company.builder()
