...
public class CustomersDBDAO implements CustomersDAO {
    ...
    public boolean isIDExists(int customerId) {
        ...
        try {
            ...
            resultset = (ResultSet) DataBaseUtils.runQueryForResult(ID_IS_EXISTS, customerFields);
            if (resultset.next()) {
                return true;
            }
        } catch (SQLException error) {
    ...
    public void deleteCustomer(int customerId) {
        try {
            Map<Integer, Object> customerFields = new HashMap<>();
            customerFields.put(1, customerId);
            DataBaseUtils.runQuery(DELETE_CUSTOMER, customerFields);
        } catch (
                SQLException error) {
            System.out.println(error.getMessage());
        }
    ...
    public Customer getOneCustomer(int customerId) throws CouponSystemException {
        try {
            Map<Integer, Object> customerFields = new HashMap<>();
            customerFields.put(1, customerId);
            ResultSet resultSet = (ResultSet) DataBaseUtils.runQueryForResult(GET_ONE_CUSTOMER, customerFields);
            resultSet.next();
            Customer customer = Customer.builder()
                    .id(resultSet.getInt("id"))
                    .email(resultSet.getString("email"))
