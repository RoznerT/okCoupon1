...
public class CouponsDBDAO implements CouponsDAO {
    ...
    public boolean IsCouponExists(int couponId) throws CouponSystemException {
        ...
        couponFields.put(1, couponId);
        try {
            resultset = (ResultSet) DataBaseUtils.runQueryForResult(GET_ONE_COUPON, couponFields);
            if (resultset.next()) {
                return true;
            }
        } catch (SQLException error) {
    ...
    public void deleteCoupon(int couponId) throws CouponSystemException {
        try {
            Map<Integer, Object> couponFields = new HashMap<>();
            couponFields.put(1, couponId);
            DataBaseUtils.runQuery(DELETE_COUPON, couponFields);
        } catch (SQLException error) {
            System.out.println(error.getMessage());
            throw new CouponSystemException(CouponErrMsg.NOT_EXISTS);
        }
    ...
    public ArrayList<Coupon> getAllCouponsOfCustomer(int customerId) throws CouponSystemException {
        ...
        customerFields.put(1, customerId);
        try {
            ResultSet resultSet = (ResultSet) DataBaseUtils.runQueryForResult(GET_COUPONS_BY_CUSTOMER, customerFields);
            while (resultSet.next()) {
    ...
    public ArrayList<Coupon> getAllCouponsOfCompany(int companyId) throws CouponSystemException {
        ...
        companyFields.put(1, companyId);
        try {
            ResultSet resultSet = (ResultSet) DataBaseUtils.runQueryForResult(GET_COUPONS_BY_COMPANY, companyFields);
            while (resultSet.next()) {
    ...
    public void deleteHistoryOfCoupons(int couponId) {
        try {
            Map<Integer, Object> couponFields = new HashMap<>();
            couponFields.put(1, couponId);
            DataBaseUtils.runQuery(DELETE_HISTORY, couponFields);
        } catch (SQLException error) {
            System.out.println(error.getMessage());
        }
    }
